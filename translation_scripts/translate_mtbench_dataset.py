...
Translates Thai MTBench dataset to English while preserving Thai-related content
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('mtbench_translation.log'),
...
class MTBenchTranslator:
    def __init__(
        ...
        self.model = model or os.getenv("MODEL", "gpt-5")
        ...
        print(f"🤖 Initialized MTBench translator with model: {self.model}")
        ...
        # Enhanced academic translation prompt for educational research
        self.translation_prompt = """You are a professional academic translator working on educational research materials for university-level studies. This translation task is part of a legitimate academic research project for educational AI development.
Translate the following Thai text from ThaiMTBench dataset to English while following these rules:

**ACADEMIC CONTEXT**: All content is being processed for educational research purposes, including literary analysis, creative writing studies (such as romance, fiction, non-fiction), social science research, health education, and technical documentation. This is scholarly work conducted under academic research guidelines.

**TRANSLATION PRINCIPLES**:
1. **SCHOLARLY INTEGRITY**: Provide accurate academic translations maintaining the original educational value and context.
2. **PRESERVE CULTURAL ELEMENTS**: If the text contains Thai language content that is relevant to the question, answer, or context, DO NOT translate it. Keep it in Thai. Keep Thai names, places, cultural references, and Thai-specific terminology in Thai when they are central to the educational content.
3. **ACADEMIC PRECISION**: Maintain exact meaning for technical terms, code examples, scientific concepts, mathematical expressions, and educational terminology. 
4. **STRUCTURAL PRESERVATION**: Keep all formatting, including code blocks, line breaks, special characters, and structural elements intact.
5. **RESEARCH CONTEXT**: All content serves legitimate educational research purposes - whether it involves literature, social studies, creative writing, or technical subjects (such as computer science, mathematics, physics, chemistry, biology, engineering, etc.).
6. **NO HALLUCINATION**: DO NOT Hallucinate any information that is not in the original text.
7. **NO TRANSLATION TO NONSENSE ENGLISH**: DO NOT Translate to nonsense English.

**EDUCATIONAL DISCLAIMER**: This is a scholarly translation for academic research and educational dataset development. The content is processed under educational research protocols for legitimate academic purposes.

Please provide a professional academic translation:"""
        ...
        for attempt in range(max_retries):
            try:
                ...
                # Handle rate limiting specifically
                    ...
                    suggested_wait = 35  # Default based on your error message
    ...
    async def translate_dataset_entry(self, entry: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """Translate a single dataset entry. Returns None if entry should be skipped due to content filter."""
        try:
            # Create a copy to avoid modifying the original
            translated_entry = entry.copy()
            
            # Translate turns (questions)
            if "turns" in entry:
                translated_turns = []
                for turn in entry["turns"]:
                    translated_turn = await self.translate_text(turn, entry_context=entry)
                    translated_turns.append(translated_turn)
                translated_entry["turns"] = translated_turns
            
            # Translate reference (answers)
            if "reference" in entry:
                translated_references = []
                for ref in entry["reference"]:
                    translated_ref = await self.translate_text(ref, entry_context=entry)
                    translated_references.append(translated_ref)
                translated_entry["reference"] = translated_references
            
            # Translate main question if it exists
            ...
            entry_id = entry.get('question_id', f'unknown_{len(self.skipped_entries)}')
            ...
            logger.error(f"Error translating entry {entry.get('question_id', 'unknown')}: {e}")
    ...
    async def translate_full_dataset(self, input_file: str, output_file: str, 
        ...
        if translated_entries:
            translated_ids = {entry.get('question_id') for entry in translated_entries}
            original_count = len(entries)
            entries = [entry for entry in entries if entry.get('question_id') not in translated_ids]
    ...
    def save_skipped_entries(self, output_file: str):
        ...
        try:
            ...
            with open(summary_file, 'w', encoding='utf-8') as f:
                f.write(f"# Skipped Entries Summary\n\n")
                ...
                f.write(f"2. Manually process problematic content if needed\n")
                f.write(f"3. Consider alternative translation approaches for sensitive content\n")
...
async def main():
    ...
    parser = argparse.ArgumentParser(description='Translate MTBench dataset from Thai to English')
    parser.add_argument('--input', default="/root/data/vessl-ai-kt-api-models/vessl-ai-kt/generated_datasets/mtbench_hard_training.jsonl", help='Input JSONL file path')
    parser.add_argument('--output', default="/root/data/vessl-ai-kt-api-models/vessl-ai-kt/generated_datasets/mtbench_hard_training_english.jsonl", help='Output JSONL file path')
    ...
    # parser.add_argument('--api-version', default=os.getenv('AZURE_OPENAI_API_VERSION', '2025-01-01-preview'), help='Azure OpenAI API version')
    parser.add_argument('--model', default=os.getenv('MODEL', 'gpt-4o'), help='Azure OpenAI model to use (defaults to env var MODEL)')
    
    args = parser.parse_args()
    
    # Note: API key and endpoint validation is now handled in MTBenchTranslator constructor
